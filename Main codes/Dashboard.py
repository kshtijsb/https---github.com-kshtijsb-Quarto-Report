...
if __name__ == "__main__":
   ...
   # 1. Line Chart: Trend of 'Proportion of health care facilities with no sanitation service' over the years for Afghanistan
    indicator_of_interest = 'Proportion of health care facilities with no sanitation service'

    # Filter data for the indicator of interest
    sanitation_data = df_indicator_1[df_indicator_1['indicator'] == indicator_of_interest]

    # Calculate average sanitation service coverage for each country
    avg_sanitation_by_country = sanitation_data.groupby('country')['obs_value'].mean()

    # Sort countries by average sanitation service coverage in descending order and select the top 10
